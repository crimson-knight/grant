...
describe "Granite::Associations::Polymorphic - Edge Cases (Standalone)" do
  describe "error handling" do
    it "handles invalid type names gracefully" do
      proxy = Granite::Polymorphic::PolymorphicProxy.new("InvalidClass", 123_i64)
      proxy.load.should be_nil
    end
    
    it "raises appropriate error with load!" do
      proxy = Granite::Polymorphic::PolymorphicProxy.new("InvalidClass", 123_i64)
      expect_raises(Granite::Querying::NotFound, /No InvalidClass found/) do
        proxy.load!
      end
    end
    
    it "handles nil proxy with load!" do
      proxy = Granite::Polymorphic::PolymorphicProxy.new(nil, nil)
      expect_raises(Granite::Querying::NotFound, /Polymorphic association not set/) do
        proxy.load!
      end
    end
  end
  ...
  describe "proxy behavior" do
    it "reloads associations" do
      proxy = Granite::Polymorphic::PolymorphicProxy.new("StandalonePost", 999_i64)
    ...
    it "correctly identifies present associations" do
      ...
      proxy1 = Granite::Polymorphic::PolymorphicProxy.new("StandalonePost", 123_i64)
      proxy1.present?.should be_true
      
      # With only type
      proxy2 = Granite::Polymorphic::PolymorphicProxy.new("StandalonePost", nil)
      proxy2.present?.should be_false
      
      # With only id
      proxy3 = Granite::Polymorphic::PolymorphicProxy.new(nil, 123_i64)
  ...
  describe "type registration verification" do
    it "registers all model types automatically" do
      Granite::Polymorphic.registered_type?("StandalonePost").should be_true
      Granite::Polymorphic.registered_type?("StandaloneArticle").should be_true
      Granite::Polymorphic.registered_type?("StandaloneComment").should be_true
      Granite::Polymorphic.registered_type?("NonExistentModel").should be_false
    end
  end
end

# Standalone test models without UUID dependencies
Granite::Connections << Granite::Adapter::Sqlite.new(name: "sqlite", url: "sqlite3::memory:")

class StandaloneComment < Granite::Base
  connection sqlite
...
class StandaloneStrictComment < Granite::Base
  connection sqlite
...
class StandaloneCustomItem < Granite::Base
  connection sqlite
...
class StandalonePost < Granite::Base
  connection sqlite
...
class StandaloneArticle < Granite::Base
  connection sqlite
...
class StandaloneInvalidPK < Granite::Base
  connection sqlite
...
class StandaloneImage < Granite::Base
  connection sqlite
