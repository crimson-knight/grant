...
describe "Granite::Associations::Polymorphic - Basic Implementation" do
  ...
  describe "type registration" do
    it "checks if types are registered" do
      ...
      Granite::Polymorphic.registered_type?("TestPost").should be_true
      Granite::Polymorphic.registered_type?("TestBook").should be_true
      Granite::Polymorphic.registered_type?("NonExistent").should be_false
    end
  end
  
  describe "polymorphic proxy" do
    it "creates a proxy for lazy loading" do
      proxy = Granite::Polymorphic::PolymorphicProxy.new("TestPost", 123_i64)
      proxy.type.should eq("TestPost")
      proxy.id.should eq(123_i64)
      proxy.present?.should be_true
      
      nil_proxy = Granite::Polymorphic::PolymorphicProxy.new(nil, nil)
