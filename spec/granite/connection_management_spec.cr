...
class ConnectionTestModel < Granite::Base
...
class UnConfiguredModel < Granite::Base
  table unconfigured
  column id : Int64, primary: true
end

describe Granite::ConnectionManagement do
  describe "connection_switch_wait_period" do
    it "delegates to Granite::Connections" do
      # Save original value
      original_value = Granite::Connections.connection_switch_wait_period
      ...
      Granite::Connections.connection_switch_wait_period.should eq 5000
      ConnectionTestModel.connection_switch_wait_period.should eq 5000
      
      # Restore original value
      Granite::Connections.connection_switch_wait_period = original_value
    end
  end
  ...
  describe "adapter selection" do
    it "selects adapter based on current context" do
      ...
      ConnectionTestModel.adapter.should be_a(Granite::Adapter::Base)
    end
    ...
    it "falls back to registered connections when not configured" do
      ...
      UnConfiguredModel.adapter.should be_a(Granite::Adapter::Base)
    end
  end
end