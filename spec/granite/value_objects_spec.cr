...
struct Money
  ...
  def validate
    ...
    errors << Granite::Error.new(:amount, "must be positive") if amount < 0
    errors << Granite::Error.new(:currency, "must be 3 characters") if currency.size != 3
    errors
  end
end
...
# Test models
class CustomerWithAddress < Granite::Base
...
class CustomerWithMoney < Granite::Base
  connection {{ adapter_literal }}
...
class CustomerWithCustomConstructor < Granite::Base
  connection {{ adapter_literal }}
...
class CustomerWithMultiple < Granite::Base
  connection {{ adapter_literal }}
...
describe Granite::ValueObjects do
  before_each do
