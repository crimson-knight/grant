...
describe Granite::Locking::Pessimistic do
  describe ".lock" do
    it "adds lock mode to query" do
      query = Parent.lock
      query.lock_mode.should eq(Granite::Locking::LockMode::Update)
    end
    
    it "accepts specific lock mode" do
      query = Parent.lock(Granite::Locking::LockMode::Share)
      query.lock_mode.should eq(Granite::Locking::LockMode::Share)
    end
  ...
  describe "lock mode SQL generation" do
    it "generates correct SQL for different adapters" do
      pg_adapter = Granite::Adapter::Pg.new("test_pg", "postgres://localhost/test")
      mysql_adapter = Granite::Adapter::Mysql.new("test_mysql", "mysql://localhost/test")
      sqlite_adapter = Granite::Adapter::Sqlite.new("test_sqlite", "sqlite3://./test.db")
      
      # PostgreSQL supports all lock modes
      Granite::Locking::LockMode::Update.to_sql(pg_adapter).should eq("FOR UPDATE")
      Granite::Locking::LockMode::UpdateNoWait.to_sql(pg_adapter).should eq("FOR UPDATE NOWAIT")
      Granite::Locking::LockMode::ShareSkipLocked.to_sql(pg_adapter).should eq("FOR SHARE SKIP LOCKED")
      
      # MySQL has limited support
      Granite::Locking::LockMode::Update.to_sql(mysql_adapter).should eq("FOR UPDATE")
      Granite::Locking::LockMode::Share.to_sql(mysql_adapter).should eq("LOCK IN SHARE MODE")
      
      # SQLite doesn't support row-level locking
      Granite::Locking::LockMode::Update.to_sql(sqlite_adapter).should eq("")
    end
  end
  ...
  describe "adapter support checks" do
    it "correctly reports lock mode support" do
      ...
      when Granite::Adapter::Pg
        Parent.adapter.supports_lock_mode?(Granite::Locking::LockMode::UpdateNoWait).should be_true
      when Granite::Adapter::Mysql
        Parent.adapter.supports_lock_mode?(Granite::Locking::LockMode::Update).should be_true
        Parent.adapter.supports_lock_mode?(Granite::Locking::LockMode::ShareNoWait).should be_false
      when Granite::Adapter::Sqlite
        Parent.adapter.supports_lock_mode?(Granite::Locking::LockMode::Update).should be_false
      end
    end
  end
end