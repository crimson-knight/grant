...
describe "Async features" do
  describe "AsyncResult" do
    it "executes async operations" do
      result = Granite::Async::Result.new { 42 }
      result.wait.should eq(42)
    end
    
    it "handles errors" do
      result = Granite::Async::Result.new { raise "test error" }
    ...
    it "supports error recovery" do
      result = Granite::Async::Result(Int32).new { raise "test error" }
      recovered = result.on_error { |e| -1 }
      recovered.should eq(-1)
    end
    
    it "supports chaining with then" do
      result = Granite::Async::Result.new { 10 }
      chained = result.then { |v| v * 2 }
      chained.wait.should eq(20)
    end
    
    it "supports timeout" do
      result = Granite::Async::Result.new do
        sleep 0.1.seconds
        42
      end
      
      expect_raises(Granite::Async::AsyncTimeoutError) do
  ...
  describe "AsyncCoordinator" do
    it "waits for multiple operations" do
      coordinator = Granite::Async::ResultCoordinator(Int32).new
      
      3.times do |i|
        result = Granite::Async::Result.new { i * 10 }
    ...
    it "reports errors from failed operations" do
      ...
      coordinator.add(Granite::Async::Result(Int32).new { 1 })
      coordinator.add(Granite::Async::Result(Int32).new { raise "error 1" })
      coordinator.add(Granite::Async::Result(Int32).new { raise "error 2" })
      
      expect_raises(Granite::Async::AsyncCoordinationError) do
  ...
  describe "Model async methods" do
    it "performs async count" do
      ...
      result.should be_a(Granite::Async::Result(Int64))
  ...
  describe "Async metrics" do
    it "tracks operation metrics" do
      Granite::Async::Metrics.reset
      
      Parent.async_count.wait
      Student.async_count.wait
      
      metrics = Granite::Async::Metrics.snapshot
