...
class TestMultiDbModel < Grant::Base
...
describe "Multi-Database Unit Tests" do
  describe "ConnectionSpec" do
    ...
    it "generates correct connection key" do
      spec1 = Grant::ConnectionRegistry::ConnectionSpec.new(
        database: "mydb",
        adapter_class: Grant::Adapter::Sqlite,
        url: "sqlite3://test.db",
        role: :reading
      ...
      spec2 = Grant::ConnectionRegistry::ConnectionSpec.new(
        database: "mydb",
        adapter_class: Grant::Adapter::Sqlite,
        url: "sqlite3://test.db",
        role: :writing,
  ...
  describe "Load Balancing Strategies" do
    it "round-robin cycles through indices" do
      strategy = Grant::RoundRobinStrategy.new
    ...
    it "least connections strategy tracks connections" do
      strategy = Grant::LeastConnectionsStrategy.new
  ...
  describe "ConnectionRegistry with establish_connections" do
    ...
    it "establishes writer and reader connections" do
      ...
      Grant::ConnectionRegistry.connection_exists?("multi_db", :writing).should be_true
      Grant::ConnectionRegistry.connection_exists?("multi_db", :reading).should be_true
    end
  end
  ...
  describe "System Health" do
    ...
    it "reports connection health status" do
      ...
      status = Grant::ConnectionRegistry.health_status
