require "../spec_helper"
require "../../src/grant/health_monitor"
require "../../src/grant/replica_load_balancer"

# Test model for connection configuration DSL
class TestConfigModel < Grant::Base
  connects_to database: "test"
  
  # TODO: Fix connection_config macro availability
  # connection_config(
...
describe "Advanced Multi-Database Support" do
  describe Grant::ConnectionRegistry::ConnectionSpec do
  ...
  describe Grant::ReplicaLoadBalancer do
    it "distributes load across replicas" do
      adapters = Array(Grant::Adapter::Base).new
      adapters << Grant::Adapter::Sqlite.new("replica1", "sqlite3::memory:").as(Grant::Adapter::Base)
      adapters << Grant::Adapter::Sqlite.new("replica2", "sqlite3::memory:").as(Grant::Adapter::Base)
    ...
    it "supports different load balancing strategies" do
      ...
      balancer = Grant::ReplicaLoadBalancer.new(adapters, Grant::RoundRobinStrategy.new)
      balancer.strategy.should be_a(Grant::RoundRobinStrategy)
      
      # Test random strategy
      balancer.strategy = Grant::RandomStrategy.new
      balancer.strategy.should be_a(Grant::RandomStrategy)
    end
  end
  ...
  describe "Integration" do
    it "establishes connections with full configuration" do
      ...
      Grant::ConnectionRegistry.connection_exists?("test_primary", :writing).should be_true
      Grant::ConnectionRegistry.connection_exists?("test_primary", :reading).should be_true
      
      # Should have load balancer for reading role
      lb = Grant::ConnectionRegistry.get_load_balancer("test_primary")
