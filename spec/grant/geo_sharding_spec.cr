...
class GeoShardedUser < Grant::Base
  connection "test"
...
class SimpleGeoModel < Grant::Base
  connection "test"
  ...
  shards_by :country, strategy: :geo,
    regions: [
      {shard: :shard_us, countries: ["US"], states: nil, cities: nil},
      {shard: :shard_eu, countries: ["GB", "DE", "FR"], states: nil, cities: nil},
      {shard: :shard_asia, countries: ["JP", "CN", "KR"], states: nil, cities: nil},
    ],
...
describe "Geo-based Sharding" do
  ...
  describe "Region context" do
    it "can use region context for queries" do
      ...
      Grant::Sharding::RegionDetermination::Context.with(country: "US", state: "CA") do
        # Context is available
        Grant::Sharding::RegionDetermination::Context.get(:country).should eq("US")
        Grant::Sharding::RegionDetermination::Context.get(:state).should eq("CA")
      end

      # Context is cleared after block
      Grant::Sharding::RegionDetermination::Context.get(:country).should be_nil
    end
  end
end
