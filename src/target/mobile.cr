# Selects the **mobile** compile target for Grant's per-target column gating.
#
...
# `:mobile` is in its `targets` list (and ungated columns are always emitted).
