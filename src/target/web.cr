# Selects the **web** compile target for Grant's per-target column gating.
#
...
# ```
# require "grant"
# require "grant/adapter/pg" # adapter selection is just a require
# require "grant/target/web" # ← sets GRANT_COMPILE_TARGET = :web
# require "./models"         # models expand AFTER the target is set
# ```
#
# With this target active, a `column ..., targets: [...]` is emitted only when
# `:web` is in its `targets` list (and ungated columns are always emitted).
