...
module Grant::QueryAnalysis
  ...
  class QueryStats
    ...
    def report
      ...
      Grant::Logs::Query.info { "Query Statistics Summary - #{@stats.size} operations, #{@stats.values.sum(&.count)} queries (#{@stats.values.sum(&.total_duration_ms)}ms)" }
      ...
      sorted_stats.each do |key, stat|
        ...
        Grant::Logs::Query.info { "Query Stats - #{model}##{operation}: #{stat.count} queries, total: #{stat.total_duration_ms.round(2)}ms, avg: #{stat.avg_duration_ms.round(2)}ms, min: #{stat.min_duration_ms.round(2)}ms, max: #{stat.max_duration_ms.round(2)}ms" }
      end
    end
  ...
  # Integration with Grant's query execution
  module Integration
    macro included
      ...
      def log_query_with_timing(sql : String, args : Array(Grant::Columns::Type), duration : Time::Span, row_count : Int32? = nil, model_name : String? = nil)
        ...
        if detector = Grant::QueryAnalysis::N1Detector.instance
        ...
        if stats = Grant::QueryAnalysis::QueryStats.instance
...
module Grant::Query::Executor
  module Shared
    include Grant::QueryAnalysis::Integration
  end
end