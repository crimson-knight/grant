...
module Grant::CompositePrimaryKey
  ...
  macro included
    ...
    def self.primary_name
      if ck = composite_key
        ...
        # will be handled by Grant::Tables module
        ""
      end
    end
  ...
  def self.find!(**keys)
    find(**keys) || raise Grant::Querying::NotFound.new("No #{name} found with keys: #{keys}")
  end
  ...
  def new_record? : Bool
    ...
    # Use the @new_record flag which is managed by Grant::Base
