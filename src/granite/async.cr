...
module Granite
  ...
  module Async
    ...
    alias AsyncResult = Granite::Async::Result
    
    # Module to be included in Granite::Base
    module ClassMethods
      # Async count
      ...
      # Async min
      def async_min(column : Symbol | String) : AsyncResult(Granite::Columns::Type)
        AsyncResult(Granite::Columns::Type).new do
          min(column)
        end
      end
      
      # Async max
      def async_max(column : Symbol | String) : AsyncResult(Granite::Columns::Type)
        AsyncResult(Granite::Columns::Type).new do
          max(column)
        end
      end
      ...
      # Async pluck
      def async_pluck(column : Symbol | String) : AsyncResult(Array(Granite::Columns::Type))
        AsyncResult(Array(Granite::Columns::Type)).new do
          pluck(column)
        end
      end
      
      # Async pick (first value)
      def async_pick(column : Symbol | String) : AsyncResult(Granite::Columns::Type?)
        AsyncResult(Granite::Columns::Type?).new do
          pick(column)
        end
      end
