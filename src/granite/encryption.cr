...
module Granite::Encryption
  ...
  module Model
    macro included
      include Granite::Encryption::QueryExtensions
      
      # Track encrypted attributes at the class level
      class_getter encrypted_attributes = {} of String => Granite::Encryption::EncryptedAttribute
    ...
    macro encrypts(attribute, deterministic = false)
      ...
      class_getter {{attribute.id}}_encrypted_attribute : Granite::Encryption::EncryptedAttribute = 
        Granite::Encryption::EncryptedAttribute.new(
          self,
          {{attr_name}},
          {{deterministic}}
        )
      ...
      def {{attribute.id}}=(value : String?)
        ...
        if value.nil?
          ...
          @{{attribute.id}}_encrypted = Granite::Encryption.encrypt(
            value,
            self.class.name,
            {{attr_name}},
            {{deterministic}}
      ...
      {% if deterministic %}
        ...
        def self.where_{{attribute.id}}(value : String)
          encrypted = Granite::Encryption.encrypt(
            value,
            self.name,
            {{attr_name}},
            true
