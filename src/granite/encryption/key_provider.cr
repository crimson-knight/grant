...
module Granite::Encryption
  ...
  class KeyProvider
    ...
    DERIVE_INFO_PREFIX = "granite-encryption"
    
    # Key size in bytes (256 bits for AES-256)
    KEY_SIZE = 32
    
    # Default key derivation salt
    DEFAULT_SALT = "granite-encryption-v1"
    ...
    def self.primary_key! : Bytes
      primary_key || raise KeyError.new("Primary encryption key not configured. Set Granite::Encryption::KeyProvider.primary_key")
    end
    
    # Get the deterministic encryption key
    def self.deterministic_key! : Bytes
      deterministic_key || raise KeyError.new("Deterministic encryption key not configured. Set Granite::Encryption::KeyProvider.deterministic_key")
    end
