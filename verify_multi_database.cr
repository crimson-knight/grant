...
class TestConfiguredModel < Grant::Base
...
begin
  spec = Grant::ConnectionRegistry::ConnectionSpec.new(
    database: "test",
    adapter_class: Grant::Adapter::Base,
    url: "sqlite3://test.db",
    role: :primary,
    pool_size: 20,
    initial_pool_size: 5,
...
begin
  # Round-robin
  rr_strategy = Grant::RoundRobinStrategy.new
  rr_indices = Array.new(6) { rr_strategy.next_index(3) }
  
  # Random
  rand_strategy = Grant::RandomStrategy.new
  ...
  # Least connections
  lc_strategy = Grant::LeastConnectionsStrategy.new
...
begin
  ...
  exists = Grant::ConnectionRegistry.connection_exists?("test_db", :primary)
  ...
  # Test health status
  health = Grant::ConnectionRegistry.health_status
